...
echo "主脚本：bash <(curl -Ls https://raw.githubusercontent.com/axe528/argosbx/main/argosbx.sh) 或 bash <(wget -qO- https://raw.githubusercontent.com/axe528/argosbx/main/argosbx.sh)"
...
upxray(){
url="https://github.com/axe528/argosbx/releases/download/argosbx/xray-$cpu"; out="$HOME/agsbx/xray"; (command -v curl >/dev/null 2>&1 && curl -Lo "$out" -# --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && timeout 3 wget -O "$out" --tries=2 "$url")
chmod +x "$HOME/agsbx/xray"
sbcore=$("$HOME/agsbx/xray" version 2>/dev/null | awk '/^Xray/{print $2}')
echo "已安装Xray正式版内核：$sbcore"
}
upsingbox(){
url="https://github.com/axe528/argosbx/releases/download/argosbx/sing-box-$cpu"; out="$HOME/agsbx/sing-box"; (command -v curl>/dev/null 2>&1 && curl -Lo "$out" -# --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && timeout 3 wget -O "$out" --tries=2 "$url")
...
echo "本地生成失败，从远程下载备用证书..."
url="https://github.com/axe528/argosbx/releases/download/argosbx/private.key"; out="$HOME/agsbx/private.key"; (command -v curl>/dev/null 2>&1 && curl -Ls -o "$out" --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && timeout 3 wget -q -O "$out" --tries=2 "$url")
url="https://github.com/axe528/argosbx/releases/download/argosbx/cert.pem"; out="$HOME/agsbx/cert.pem"; (command -v curl>/dev/null 2>&1 && curl -Ls -o "$out" --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && timeout 3 wget -q -O "$out" --tries=2 "$url")
fi
echo "self" > "$HOME/agsbx/cert_type"
TLS_SNI="www.bing.com"
TLS_INSECURE="true"
echo "✅ 自签证书配置完成，客户端需设置 skip-cert-verify: true"
}
...
echo "=========TLS证书配置========="
if [ -f "$HOME/agsbx/cert.pem" ] && [ -f "$HOME/agsbx/private.key" ]; then
CERT_TYPE=$(cat "$HOME/agsbx/cert_type" 2>/dev/null)
if [ "$CERT_TYPE" = "real" ]; then
TLS_SNI=$(cat "$HOME/agsbx/cert_domain" 2>/dev/null)
TLS_INSECURE="false"
else
TLS_SNI="www.bing.com"
TLS_INSECURE="true"
fi
echo "检测到已有证书（类型：${CERT_TYPE:-self}），跳过申请"
return 0
fi
if [ -n "$cert_domain" ] && [ -n "$cf_token" ]; then
echo "检测到证书环境变量，使用 Let's Encrypt 证书"
echo "域名：$cert_domain"
apply_real_certificate
return
fi
echo "请选择 TLS 证书类型："
echo "  1. Let's Encrypt 真实证书（需要域名 + Cloudflare API Token）"
echo "  2. 自签证书（无需域名，客户端需跳过证书验证）[默认]"
echo
printf "请输入选项 [1/2，直接回车默认选2]："
read -r cert_choice
if [ "$cert_choice" = "1" ]; then
echo
printf "请输入你的域名（如 proxy.example.com）："
read -r cert_domain
...
echo
echo "获取 Cloudflare API Token 步骤："
echo "  1. 登录 Cloudflare → 右上角头像 → My Profile"
echo "  2. 左侧 API Tokens → Create Token"
echo "  3. 选择模板 Edit zone DNS"
echo "  4. 权限：Zone-DNS-Edit + Zone-Zone-Read，选择对应域名"
echo
printf "请输入 Cloudflare API Token："
read -r cf_token
if [ -z "$cf_token" ]; then
echo "API Token 为空，使用自签证书"
apply_self_signed_certificate
return
fi
apply_real_certificate
...
CERT_TYPE=$(cat "$HOME/agsbx/cert_type" 2>/dev/null)
if [ "$CERT_TYPE" = "real" ]; then
CERT_SNI=$(cat "$HOME/agsbx/cert_domain" 2>/dev/null)
CERT_INSECURE="false"
CERT_SKIP="false"
else
...
port_an=$(cat "$HOME/agsbx/port_an")
if [ "$CERT_TYPE" = "real" ]; then
an_link="anytls://$uuid@$server_ip:$port_an?security=tls&sni=${CERT_SNI}&insecure=0&allowInsecure=0#${sxname}anytls-$hostname"
else
...
hy2_link="hysteria2://$uuid@$server_ip:$port_hy2?security=tls&alpn=h3&insecure=1&allowInsecure=1$hyps&sni=${CERT_SNI}#${sxname}hy2-$hostname"
...
echo "💣【 Tuic 】节点信息如下："
port_tu=$(cat "$HOME/agsbx/port_tu")
tuic5_link="tuic://$uuid:$uuid@$server_ip:$port_tu?congestion_control=bbr&udp_relay_mode=native&alpn=h3&sni=${CERT_SNI}&insecure=1&allowInsecure=1&allow_insecure=1#${sxname}tuic-$hostname"
...
cleandel
rm -rf "$HOME/agsbx"/{sb.json,xr.json,sbargoym.log,sbargotoken.log,argo.log,argoport.log,cdnym,name,cert_type,cert_domain}
